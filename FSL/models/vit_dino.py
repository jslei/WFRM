...
class VisionTransformer(nn.Module):
    """ Vision Transformer """
    def __init__(self, lower_fea = 0, concate=True, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        self.depth = depth
        # self.lower_fea = lower_fea # collect features generated by the last i-th block

        # self.concate = concate

        # if self.concate:
        #     self.norm_concate = norm_layer(embed_dim*self.lower_fea)
        #     self.out_dim = embed_dim*self.lower_fea
        # else:
        #     self.out_dim = embed_dim

        # if lower_fea>0:
        #     self.ensemble_weight = nn.ParameterList([torch.zeros(1,embed_dim)]*(lower_fea-1)+[torch.ones(1,embed_dim)])
    ...
    def forward(self, x, prefix=None, return_feat=False, use_adapter=True):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            # if self.depth-i-1<self.lower_fea:
            #     features.append(x[:,0])

        # if self.lower_fea>0:
        #     if self.concate:
        #         y = []
        #         for i in range(self.lower_fea):
        #             y.append(features[i]*self.ensemble_weight[i])
        #         y = torch.cat(y,dim=1)
        #         return self.norm_concate(y)
        #     else:
        #         y = 0
        #         for i in range(self.lower_fea):
        #             y += features[i]*self.ensemble_weight[i]
        #         return self.norm(y)
